"""Generate a complete Moore starter .docx: cover page + body section ready for body text."""
...
PLACEHOLDER_TEXT = (
    "[Paste Moore article body here — apply TME Body style to paragraphs, "
    "TME H1/H2/H3 to headings, TME Figure Caption to captions, "
    "TME Reference to reference entries.]"
)


def build_moore_starter(*, out_path: Path, headshots: Dict[str, Path]) -> None:
    """Generate a complete starter .docx with:
    - Page 1: Moore cover page (masthead, tagline, title, authors, abstract, author block,
      cover footer) using the 3-column author row.
    - A NEW_PAGE section break.
    - Page 2+: empty body section with running headers and footer pre-set, plus a
      placeholder paragraph styled as TME Body.
    ...
    # --- Section 0: masthead + tagline, full-bleed (zero margins) ---
    ...
    # Cover footer belongs on Section 0 (unambiguously page 1 with zero margins).
    # Placing it here avoids Word's footer-resolution ambiguity when continuous
    # section breaks share the same physical page.
    add_cover_footer(
        masthead_section,
    ...
    # Unlink Section 1's footer so it doesn't inherit (or re-display) the cover footer.
    ...
    # --- Section break: NEW_PAGE starts the body section (Section 2) ---
    body_section = add_section_break_next_page(doc)

    # Running headers: verso = author cite, recto = short title
    set_running_headers(
        doc,
        author_cite="Moore, Yasuda, & Wong",
